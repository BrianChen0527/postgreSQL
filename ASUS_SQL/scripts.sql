...
SELECT employeeID, SUM(price) FROM Sale JOIN Item ON Sale.SaleID=Item.SaleID GROUP BY employeeID
) a
WHERE sum = (
SELECT MIN(a.sum) FROM (SELECT employeeID, SUM(price) FROM Sale JOIN Item ON Sale.SaleID=Item.SaleID GROUP BY employeeID) a
);
...
SELECT CityID, SUM(Price) FROM Employee JOIN sale ON Employee.employeeID=Sale.employeeID 
JOIN Item ON sale.saleID=Item.saleID GROUP BY CityID;

-- list out the sales of a specific item in each city (here we use 'LEXUS' as an example item)
SELECT CityID, SUM(price) FROM (Employee JOIN sale ON Employee.employeeID=Sale.employeeID 
JOIN Item ON sale.saleID=Item.saleID) WHERE ItemName='Lexus' GROUP BY CityID;


-- list out the item sales in a specific city (here we use 'TAIPEI' as an example city)
SELECT itemName, SUM(price) FROM (Employee JOIN sale ON Employee.employeeID=Sale.employeeID 
JOIN Item ON sale.saleID=Item.saleID) WHERE CityID='Taipei' GROUP BY itemName ORDER BY sum DESC; 
...
SELECT customerID, AVG(price) FROM Sale JOIN Item ON Sale.SaleID=Item.SaleID GROUP BY customerID HAVING AVG(price) > 
(SELECT AVG(price) FROM (Employee JOIN sale ON Employee.employeeID=Sale.employeeID 
JOIN Item ON sale.saleID=Item.saleID) WHERE CityID='Taipei');
